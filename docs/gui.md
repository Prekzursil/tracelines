...
The **[cockpit on GitHub Pages](https://prekzursil.github.io/svcoverage/)** is a static control
...
| **🛠️ Command builder** | Draw a bbox → get the exact `svcoverage extract …` command with a Copy button. Run it locally, then drop the result back in. | the CLI, run locally |
...
| **🔵 Live Google (proxy)** | Live Google extraction via a backend **you** run. | the [self-hosted proxy](https://github.com/Prekzursil/svcoverage/tree/main/server) + its URL |

## The cockpit loop

```
draw bbox  →  copy CLI command  →  run svcoverage locally  →  drag the GeoJSON back in
...
- To use **Live Google**, run the proxy yourself and lock its CORS to your GUI origin (see the
  [proxy README](https://github.com/Prekzursil/svcoverage/tree/main/server)).
